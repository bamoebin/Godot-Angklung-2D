...
public partial class TransformasiFast : RefCounted
{
	...
	public List<Vector2> Mirror(List<Vector2> points, int axis, Vector2 center)
{
	// axis: 0 untuk sumbu X, 1 untuk sumbu Y
	List<Vector2> mirroredPoints = new List<Vector2>();

	foreach (var point in points)
	{
		if (axis == 0) // Mirror terhadap sumbu X
		{
			float mirroredY = 2 * center.Y - point.Y;
			mirroredPoints.Add(new Vector2(point.X, mirroredY));
		}
		else if (axis == 1) // Mirror terhadap sumbu Y
		{
			float mirroredX = 2 * center.X - point.X;
			mirroredPoints.Add(new Vector2(mirroredX, point.Y));
		}
	}
